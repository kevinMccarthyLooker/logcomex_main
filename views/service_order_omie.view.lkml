view: service_order_omie {
  ...
  dimension: omie_so_product_normalized {
    ...
    sql:  CASE WHEN (${TABLE}."service_description" in ('LEADS  OC.: 18854   Logcomex (Nakata   Osasco / SP)', 'LEADS', 'LEADS OC.: 18854 Logcomex (Nakata Osasco / SP)', 'LEADS - Acrescimo de download para excel')) THEN 'Leads'
           ...
           WHEN (${TABLE}."service_description" in ('BIG DATA  - Importacao Maritima', 'BIG DATA  VENCIMENTO DIA 30', 'BIG DATA - Importacao Maritima - adicao de portos', 'BIG DATA - Importacao Maritima', 'BIG DATA Exportacao', 'BIG DATA', 'BIG DATA  Impo Aerea', 'BIG DATA - Importacao Maritima; Importacao Aerea;', 'BIG DATA - Importacao maritima', 'BIG DATA - Importacao Aerea', 'BIG DATA - Slim importacao.')) THEN 'BigData'
